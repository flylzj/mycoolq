...
from .resource import RollEvent
from .model import count_today_roll
# from coolq.util.coolq import get_user_id_group_id
# from coolq.new_plugins.roll.resource import RollEvent
...
roll_command = on_command("roll", aliases={"肉"})


@roll_command.handle()
async def roll_handle(bot: Bot, event: Event, state: T_State):
    # group_id, user_id = get_user_id_group_id(event.get_session_id())
    # count = count_today_roll(group_id=group_id, user_id=user_id)
    await roll_command.finish("你肉到了{}点".format(random.randint(1, 6)))
    # if (group_id in plugin_config.MANAGING_GROUPS and count <= 1) or group_id not in plugin_config.MANAGING_GROUPS:
    ...
    #     # await roll_command.finish(message)
