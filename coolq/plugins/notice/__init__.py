...
from config import NOTICE_GROUP, NEED_SIGN_USER
...
async def sign(session: CommandSession):
    ...
    if not has_signed_today(user_id):
        ...
        message += "已经打过卡了"
    await session.bot.send_group_msg(group_id=NOTICE_GROUP, message=message)


@nonebot.scheduler.scheduled_job('cron', hour="8,17,21", minute="*")
async def notice_sign():
    for user_id in NEED_SIGN_USER:
        if not has_signed_today(user_id):
            bot = nonebot.get_bot()
            message = at_someone(user_id)
            await bot.send_group_msg(group_id=NOTICE_GROUP, message=message)